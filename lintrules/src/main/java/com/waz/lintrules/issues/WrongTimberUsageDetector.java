...
public class WrongTimberUsageDetector extends Detector implements Detector.JavaScanner {
  ...
  public static final Issue ISSUE_D_USAGE =
      Issue.create(
          "com.waz.TimberDDetector",
  ...
  public static final Issue ISSUE_THROWABLE =
      Issue.create(
          "com.waz.ThrowableNotAtBeginning",
  ...
  public static final Issue ISSUE_BINARY =
      Issue.create(
          "com.waz.BinaryOperationInTimber",
          "Use String#format()",
  ...
  public void visitMethod(
      @NonNull JavaContext context, AstVisitor visitor, @NonNull MethodInvocation node) {
    ...
    if ("format".equals(methodName)) {
      ...
      if (node.astOperand() instanceof VariableReference) {
        ...
        if ("Log".equals(ref.astIdentifier().astValue())) {
          context.report(
              ISSUE_LOG, node, context.getLocation(node), "Using 'Log' instead of 'Timber'");
          return;
        }
        String callName = node.astName().astValue();
        if ("d".equals(callName)) {
          String message =
              String.format("Use another method from timber.log.Timber or remove %1$s()", callName);
      ...
      context.report(
          ...
          String.format(
              "Wrong argument count, format string `%1$s` requires `%2$d` but format "
                  + "call supplies `%3$d`",
              formatString, argumentCount, passedArgCount));
      return;
    }
        ...
        if (!valid) {
          String message =
              String.format(
                  ...
                  i, formatString, formatType, type.getSimpleName(), startIndexOfArguments + i + 1);
  ...
  private boolean isSubclassOf(
      JavaContext context, VariableReference variableReference, Class clazz) {
    ...
    if (resolved instanceof JavaParser.ResolvedVariable) {
      JavaParser.ResolvedClass typeClass =
          ((JavaParser.ResolvedVariable) resolved).getType().getTypeClass();
