...
public class ObjectAnimatorPropertyDetector extends Detector implements Detector.JavaScanner {
  ...
  public void visitMethod(JavaContext context, AstVisitor visitor, MethodInvocation node) {
    ...
    if (property instanceof VariableReference) {
      if (!isSubclassOf(context, (VariableReference) property, "android.util.Property")
          && !isSubclassOf(
              context, (VariableReference) property, "com.nineoldandroids.util.Property")) {
        context.report(
            ISSUE,
            context.getLocation(node),
            String.format(
                "'%s' should be replaced with a property",
  ...
  private boolean isSubclassOf(
      JavaContext context, VariableReference variableReference, String clazz) {
    JavaParser.ResolvedField resolvedVar =
        (JavaParser.ResolvedField) context.resolve(variableReference);
    if (resolvedVar == null) {
      return false;
    }
