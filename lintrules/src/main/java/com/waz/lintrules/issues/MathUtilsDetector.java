...
public class MathUtilsDetector extends Detector implements Detector.JavaScanner {
  ...
  private class FloatEqualVisitor extends ForwardingAstVisitor {
    ...
    private boolean verifyExpression(@NonNull BinaryExpression node) {
      ...
      if (leftType instanceof FloatingPointLiteral && rightType instanceof FloatingPointLiteral) {
        return true;
      }

      if (leftType instanceof FloatingPointLiteral && rightType instanceof IntegralLiteral) {
        return true;
      }

      return rightType instanceof FloatingPointLiteral && leftType instanceof IntegralLiteral;
    }
    ...
    private Literal getType(Expression expression) {
      ...
      if (expression instanceof VariableReference || expression instanceof MethodInvocation) {
