...
public class SystemNotificationListenerService extends NotificationListenerService {
  ...
  public void onNotificationPosted(StatusBarNotification statusBarNotification) {
    ...
    if (isWireNotification(packageName)) {
      recentNotificationMessage =
          getNotificationMessage(
              statusBarNotification.getId(), statusBarNotification.getNotification());
    }
  }
      ...
      switch (command) {
        ...
        default:
          throw new RuntimeException(
              String.format("Cannot identify the notification command [%s]", command));
      }
    }
  }
}
