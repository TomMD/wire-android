...
public class ModelObserverTest {
  ...
  public void nullModelsShouldNotTriggerAnUpdate() {
    ...
    assertEquals(
        "The observer's update method should never have called", 0, observer.callsToUpdate);
  }
  ...
  public void setAndUpdateRemovesOldModels() {
    ...
    assertEquals(
        "The observer should be listening to modelNew", modelNew, observer.getModels().first());
  }
  ...
  public void addAndUpdateOnlyAddsNonDuplicateModelsAndDoesntTriggerUpdate() {
    ...
    assertEquals(
        "The observer should only be listening to one model", 1, observer.getModels().size());
    assertEquals(
        "The observer's update method should only have been called once",
        1,
        observer.callsToUpdate);
  }
  ...
  public void setAndUpdateNewModelsShouldBeUpdated() {
    ...
    assertEquals(
        "The observer's update method should only have been called once more for the new model",
        3,
        observer.callsToUpdate);
  }
  ...
  public void forceUpdatingPassesUpdateReason() {
    ...
    assertEquals(shouldHaveBeenCalled("twice"), 2, observer.callsToUpdate);
    assertEquals(
        updateReasonShouldHaveBeen(FORCED_UPDATE), FORCED_UPDATE, observer.lastUpdateReasons.pop());
  }
  ...
  public void internalModelUpdatePassesUpdateReason() {
    ...
    assertEquals(shouldHaveBeenCalled("twice"), 2, observer.callsToUpdate);
    assertEquals(
        updateReasonShouldHaveBeen(INTERNAL_CHANGE),
        INTERNAL_CHANGE,
        observer.lastUpdateReasons.pop());
  }
  ...
  public void assigningNewModelPassesUpdateReason() {
    ...
    assertEquals(shouldHaveBeenCalled("twice"), 2, observer.callsToUpdate);
    assertEquals(
        updateReasonShouldHaveBeen(NEW_MODEL), NEW_MODEL, observer.lastUpdateReasons.pop());
  }
