...
/**
 * {@link TouchDelegate} that gates {@link MotionEvent} instances by comparing then against
 * fractional dimensions of the source view.
 *
 * <p>This is particularly useful when you want to define a rectangle in terms of the source
 * dimensions, but when those dimensions might change due to pending or future layout passes.
 *
 * <p>One example is catching touches that occur in the top-right quadrant of {@code sourceParent},
 * and relaying them to {@code targetChild}. This could be done with: <code>
...
public class FractionalTouchDelegate extends TouchDelegate {
  ...
  /**
   * Helper to create and setup a {@link FractionalTouchDelegate} between the given {@link View}.
   *
   * @param source Larger source {@link View}, usually a parent, that will be assigned {@link
   *     View#setTouchDelegate(TouchDelegate)}.
   * @param target Smaller target {@link View} which will receive {@link MotionEvent} that land in
   *     requested fractional area.
   * @param sourceFraction Fractional area projected onto source {@link View} which determines when
   *     {@link MotionEvent} will be passed to target {@link View}.
   */
  ...
  /** Consider updating {@link #sourcePartial} when {@link #source} dimensions have changed. */
