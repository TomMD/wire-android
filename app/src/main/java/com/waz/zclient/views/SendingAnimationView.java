...
public class SendingAnimationView extends View implements ValueAnimator.AnimatorUpdateListener {
  ...
  private void init(AttributeSet attrs) {
    ...
    if (attrs != null) {
      ...
      if (ta != null) {
        ...
        backgroundColor =
            ta.getColor(R.styleable.SendingAnimationView_fullCircleColor, backgroundColor);
        backgroundCircleRadius =
            ta.getDimensionPixelSize(
                R.styleable.SendingAnimationView_backgroundCircleRadius,
                DEFAULT_ANIMATION_DURATION);
        backgroundCircleColor =
            ta.getColor(R.styleable.SendingAnimationView_backgroundColor, backgroundCircleColor);
        backgroundCirclePadding =
            ta.getDimensionPixelSize(R.styleable.SendingAnimationView_circlePadding, 0);
        inputRadius =
            ta.getDimensionPixelSize(R.styleable.SendingAnimationView_circleRadius, radius);
        pendingAngleStop =
            ta.getInt(R.styleable.SendingAnimationView_pendingStop, pendingAngleStop);
        animationDuration =
            ta.getInt(R.styleable.SendingAnimationView_pendingDuration, animationDuration);
        ta.recycle();
      }
    }
  ...
  private void initAnimations() {
    ...
    circleStartAnimator.setDuration(
        (long) (animationDuration * (CIRCLE_FILL_ANIMATION_PART / 4f * 3)));
    circleStartAnimator.setInterpolator(new LinearInterpolator());
    circleStartAnimator.addListener(
  ...
  private void recalculateRect() {
    ...
    rectF.set(
        strokeWidth + getPaddingLeft() + backgroundCirclePadding,
  ...
  protected void onDraw(Canvas canvas) {
    super.onDraw(canvas);
    if (MathUtils.floatEqual(rectF.centerX(), 0f) || MathUtils.floatEqual(rectF.centerY(), 0f)) {
      recalculateRect();
    }
    ...
    canvas.drawCircle(
        centerX,
        centerY,
        backgroundCircleRadius == DEFAULT_RADIUS
            ? radius + strokeWidth + backgroundCirclePadding
            : backgroundCircleRadius,
