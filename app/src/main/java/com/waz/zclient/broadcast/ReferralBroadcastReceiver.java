        ...
        if (token != null) {
          SharedPreferences prefs =
              context.getSharedPreferences(
                  UserPreferencesController.USER_PREFS_TAG, Context.MODE_PRIVATE);
          if (token.startsWith(PERSONAL_INVITE_PREFIX)) {
            prefs
                .edit()
                .putString(
                    UserPreferencesController.USER_PREFS_PERSONAL_INVITATION_TOKEN,
                    token.substring(PERSONAL_INVITE_PREFIX.length()))
                .apply();
          } else {
            prefs
                .edit()
                .putString(UserPreferencesController.USER_PREFS_REFERRAL_TOKEN, token)
                .apply();
          }
        }
      }
