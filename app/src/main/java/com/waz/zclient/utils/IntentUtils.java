...
public class IntentUtils {
  ...
  public static final String EXTRA_LAUNCH_FROM_SAVE_IMAGE_NOTIFICATION =
  ...
  private static final String GOOGLE_MAPS_WEB_LINK =
      "http://maps.google.com/maps?z=%d&q=loc:%f+%f+(%s)";
  ...
  public static PendingIntent getGalleryIntent(Context context, URI uri) {
    ...
    galleryIntent.setClipData(
        new ClipData(null, new String[] {IMAGE_MIME_TYPE}, new ClipData.Item(androidUri)));
  ...
  public static Intent getDebugReportIntent(Context context, URI fileUri) {
    ...
    intent.putExtra(
        Intent.EXTRA_SUBJECT, context.getString(R.string.debug_report__title, versionName));
    return intent;
  }
  ...
  public static Intent getSavedImageShareIntent(Context context, URI uri) {
    ...
    return Intent.createChooser(
        shareIntent, context.getString(R.string.notification__image_saving__action__share));
  }
  ...
  public static Intent getGoogleMapsIntent(
      Context context, float lat, float lon, int zoom, String name) {
    Uri gmmIntentUri;
  ...
  private static Intent getGoogleMapsWebFallbackIntent(
      float lat, float lon, int zoom, String name) {
    ...
    String url =
        String.format(Locale.getDefault(), GOOGLE_MAPS_WEB_LINK, zoom, lat, lon, urlEncodedName);
