...
public class KeyboardVisibilityListener {
  ...
  public KeyboardVisibilityListener(View contentView) {
    ...
    this.statusAndNavigationBarHeight =
        ContextUtils.getNavigationBarHeight(contentView.getContext())
            + ContextUtils.getStatusBarHeight(contentView.getContext());
  }
  ...
  public synchronized void onLayoutChange() {
    int newKeyboardHeight =
        Math.max(0, KeyboardUtils.getKeyboardHeight(contentView) - statusAndNavigationBarHeight);
