...
public class ZTimeFormatter {
  ...
  public static String getSeparatorTime(
      @Nullable Context context,
      LocalDateTime now,
      LocalDateTime then,
      boolean is24HourFormat,
      ZoneId timeZone,
      boolean epocIsJustNow) {
  ...
  public static String getSeparatorTime(
      ...
      boolean epocIsJustNow,
      boolean showWeekday) {
    return getSeparatorTime(
        context, now, then, is24HourFormat, timeZone, epocIsJustNow, showWeekday, false);
  }

  private static String getSeparatorTime(
      @Nullable Context context,
      LocalDateTime now,
      LocalDateTime then,
      boolean is24HourFormat,
    ...
    final boolean isLastTwoMins =
        now.minusMinutes(2).isBefore(then)
            || (epocIsJustNow && then.atZone(timeZone).toInstant().toEpochMilli() == 0);
    ...
    final String time =
        is24HourFormat
            ? res.getString(R.string.timestamp_pattern__24h_format)
    ...
    if (isSameDay) {
      ...
      if (showWeekday) {
        ...
        pattern =
            res.getString(R.string.timestamp_pattern__date_and_time__no_year_no_weekday, time);
      }
    } else {
      if (showWeekday) {
        ...
        pattern =
            res.getString(R.string.timestamp_pattern__date_and_time__with_year_no_weekday, time);
      }
    }
    try {
      ...
      if (!defaultLocale) {
        return getSeparatorTime(
            context, now, then, is24HourFormat, timeZone, epocIsJustNow, showWeekday, true);
      } else {
        return "";
      }
    }
  ...
  private static String getSingleMessageTime(Context context, Date date, boolean defaultLocale) {
    ...
    final String pattern =
        is24HourFormat
            ? resources.getString(R.string.timestamp_pattern__24h_format)
            : resources.getString(R.string.timestamp_pattern__12h_format);
    try {
      DateTimeFormatter formatter = DateTimeFormatter.ofPattern(pattern);
      return formatter.format(
          DateConvertUtils.asLocalDateTime(date).atZone(ZoneId.systemDefault()));
    } catch (Exception e) {
      ExceptionHandler.saveException(
          e,
          Thread.currentThread(),
  ...
  private static String getCurrentWeek(Context context, boolean defaultLocale) {
    ...
    try {
      DateTimeFormatter formatter = DateTimeFormatter.ofPattern(pattern);
      return formatter.format(
          DateConvertUtils.asLocalDateTime(Instant.now()).atZone(ZoneId.systemDefault()));
    } catch (Exception e) {
      ExceptionHandler.saveException(
          e,
          Thread.currentThread(),
