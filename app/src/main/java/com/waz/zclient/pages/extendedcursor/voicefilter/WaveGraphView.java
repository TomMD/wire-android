...
public class WaveGraphView extends View {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    // We draw multiple sinus waves, with equal phases but altered amplitudes, multiplied by a
    // parable function.
    for (int i = 0; i < numberOfWaves; i++) {
      ...
      // Progress is a value between 1.0 and -0.5, determined by the current wave idx, which is used
      // to alter the wave's amplitude.
        ...
        float y =
            (float)
                (scaling
                        * maxAmplitude
                        * normedAmplitude
                        * Math.sin(2f * Math.PI * (x / width) * this.frequency + this.phase)
                    + halfHeight);
      ...
      int color =
          Color.argb(newAlpha, Color.red(waveColor), Color.green(waveColor), Color.blue(waveColor));
