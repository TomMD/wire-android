 ...
 * Lenny added:
 *
 * <p>property sliding enabled flag property dimming enabled flag
 *
 * <p>SlidingPaneLayout provides a horizontal, multi-pane layout for use at the top level of a UI. A
 * left (or first) pane is treated as a content list or browser, subordinate to a primary detail
 * view for displaying content.
 *
 * <p>
 *
 * <p>Child views may overlap if their combined width exceeds the available width in the
 * SlidingPaneLayout. When this occurs the user may slide the topmost view out of the way by
 * dragging it, or by navigating in the direction of the overlapped view using a keyboard. If the
 * content of the dragged child view is itself horizontally scrollable, the user may grab it by the
 * very edge.
 *
 * <p>
 *
 * <p>Thanks to this sliding behavior, SlidingPaneLayout may be suitable for creating layouts that
 * can smoothly adapt across many different screen sizes, expanding out fully on larger screens and
 * collapsing on smaller screens.
 *
 * <p>
 *
 * <p>SlidingPaneLayout is distinct from a navigation drawer as described in the design guide and
 ...
 * <p>
 *
 * <p>Like {@link android.widget.LinearLayout LinearLayout}, SlidingPaneLayout supports the use of
 * the layout parameter <code>layout_weight</code> on child views to determine how to divide
 * leftover space after measurement is complete. It is only relevant for width. When views do not
 * overlap weight behaves as it does in a LinearLayout.
 *
 * <p>
 *
 * <p>When views do overlap, weight on a slideable pane indicates that the pane should be sized to
 * fill all available space in the closed state. Weight on a pane that becomes covered indicates
 * that the pane should be sized to fill all available space except a small minimum strip that the
 * user may use to grab the slideable view and pull it back over into a closed state.
 */
...
public class SlidingPaneLayout extends ViewGroup {
  ...
  private final List<DisableLayerRunnable> postedRunnables = new ArrayList<DisableLayerRunnable>();
  ...
  public SlidingPaneLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    dragHelper.setMinVelocity(
        MIN_FLING_VELOCITY * context.getResources().getDisplayMetrics().density);
  }

  public void setSideBarWidth(Configuration newConfig) {
    overhangSize =
        ViewUtils.toPx(getContext(), newConfig.screenWidthDp)
            - getContext().getResources().getDimensionPixelSize(R.dimen.framework__sidebar_width);
    requestLayout();
  }

  /**
   * Set a distance to parallax the lower pane by when the upper pane is in its fully closed state.
   * The lower pane will scroll between this position and its fully open state.
   *
  ...
  public boolean onTouchEvent(MotionEvent ev) {
    ...
    switch (action & MotionEventCompat.ACTION_MASK) {
      ...
      case MotionEvent.ACTION_UP:
        if ((closeOnClick && isOpen()) || isDimmed(slideableView)) {
          x = ev.getX();
          y = ev.getY();
  ...
  /**
   * Check if the layout is completely open. It can be open either because the slider itself is open
   * revealing the left pane, or if all content fits without sliding.
   *
   ...
   * @deprecated Renamed to {@link #setShadowDrawableLeft(Drawable d)} to support LTR (left to right
   *     language) and {@link #setShadowDrawableRight(Drawable d)} to support RTL (right to left
  ...
  /**
   * @deprecated Set a drawable to use as a shadow cast by the right pane onto the left pane during
   *     opening/closing.
  ...
  private void parallaxOtherViews(float slideOffset) {
    ...
    final boolean dimViews =
        slideLp.dimWhenOffset && (isLayoutRtl ? slideLp.rightMargin : slideLp.leftMargin) <= 0;
    final int childCount = getChildCount();
    for (int i = 0; i < childCount; i++) {
  ...
  private class DragHelperCallback extends ViewDragHelper.Callback {
    ...
    public int clampViewPositionHorizontal(View child, int left, int dx) {
      ...
      if (isLayoutRtlSupport()) {
        int startBound =
            getWidth() - (getPaddingRight() + lp.rightMargin + slideableView.getWidth());
