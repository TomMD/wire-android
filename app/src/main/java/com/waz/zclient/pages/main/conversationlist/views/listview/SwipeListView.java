...
public class SwipeListView extends RecyclerView implements PullForActionView {
  ...
  private void init() {
    ...
    listRowMenuIndicatorMaxSwipeOffset =
        getContext()
            .getResources()
            .getDimensionPixelSize(R.dimen.list__menu_indicator__max_swipe_offset);
  }
  ...
  /**
   * onInterceptTouchEvent is not called on move even it returns false. (Bug?) This function
   * determines the hit element and the movement.
   */
  @Override
  ...
  /**
   * Retrieves the child of the list view that is hit by the touch event. If it is the first item or
   * a disabled one, targetView is set to null.
   */
      ...
      if (targetRect.contains(x, y)) {
        ...
        if (allowSwipe) {
          ...
          try {
            ...
            targetView.setMaxOffset(
                allowSwipeAway ? viewWidth / 2 : listRowMenuIndicatorMaxSwipeOffset);
  ...
  /**
   * The BouncingListVieContainer is notified from this spot. At this place the default
   * MaxOverscrollY can be overwritten.
   */
  @Override
