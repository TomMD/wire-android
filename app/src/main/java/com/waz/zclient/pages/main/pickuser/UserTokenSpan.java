...
public class UserTokenSpan extends SpannableEditText.TokenSpan {
  ...
  public UserTokenSpan(
      String userId, String userName, Context context, boolean deleteMode, int lineWidth) {
    this.userId = userId;
    ...
    paddingHorizontal =
        context
            .getResources()
            .getDimensionPixelSize(R.dimen.people_picker__input_person_token__padding_horizontal);
    paddingRightFix =
        context
            .getResources()
            .getDimensionPixelSize(R.dimen.people_picker__input_person_token__padding_right_fix);
    textOffsetTop = 0;
    spaceForCursor =
        context
            .getResources()
            .getDimensionPixelSize(
                R.dimen.people_picker__input_person_token__extra_space_for_cursor);
    backgroundHeight = context.getResources().getDimensionPixelSize(R.dimen.wire__padding__16);
    backgroundBottomPadding =
        context.getResources().getDimensionPixelSize(R.dimen.wire__padding__4);
    backgroundRadius = backgroundBottomPadding + backgroundHeight / 2;

    int tokenBackgroundColor =
        ContextUtils.getColorWithTheme(
            R.color.people_picker__input_person_token__background_color, context);
    ...
    textPaint.setTypeface(
        TypefaceUtils.getTypeface(
            context.getResources().getString(R.string.wire__typeface__regular)));
    ...
    deleteModeTextPaint.setTypeface(
        TypefaceUtils.getTypeface(
            context.getResources().getString(R.string.wire__typeface__light)));
  ...
  public void draw(
    ...
    CharSequence truncatedText =
        TextUtils.ellipsize(userName, textPaint, availableTextSpace(), TextUtils.TruncateAt.END);
