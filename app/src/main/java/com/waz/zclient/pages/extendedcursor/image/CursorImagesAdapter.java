...
class CursorImagesAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
  ...
  private CursorCameraLayout.Callback cameraCallback =
      new CursorCameraLayout.Callback() {
        @Override
  ...
  private void load() {
    new AsyncTask<Void, Void, Cursor>() {
      ...
      protected Cursor doInBackground(Void... params) {
        ...
        c
            .moveToLast(); // force cursor loading and move to last, as we are displaying images in
                           // reverse order
        return c;
      }
  ...
  public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    if (viewType == VIEW_TYPE_CAMERA) {
      cameraViewHolder =
          new CameraViewHolder(inflater.inflate(R.layout.item_camera_cursor, parent, false));
      cameraViewHolder.getLayout().setCallback(cameraCallback);
      return cameraViewHolder;
    } else {
      return new GalleryItemViewHolder(
          (ImageAssetView) inflater.inflate(R.layout.item_cursor_gallery, parent, false));
    }
  }
  ...
  public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
    if (getItemViewType(position) == VIEW_TYPE_GALLERY) {
      ...
      ((GalleryItemViewHolder) holder)
          .setPath(cursor.getString(cursor.getColumnIndex(MediaStore.Images.Media.DATA)));
