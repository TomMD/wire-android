...
public class CameraFragment extends BaseFragment<CameraFragment.Container>
    implements CameraPreviewObserver,
  ...
  public Animation onCreateAnimation(int transit, boolean enter, int nextAnim) {
    ...
    if (nextAnim == R.anim.camera__from__profile__transition) {
      ...
      return new ProfileToCameraAnimation(
          enter,
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    intentsManager =
        new AssetIntentsManager(
            getActivity(),
            new AssetIntentsManager.Callback() {
              @Override
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup c, Bundle savedInstanceState) {
    ...
    cameraBottomControl.setOnClickListener(
    ...
    cameraControlAnimationDuration =
        getResources().getInteger(R.integer.camera__control__ainmation__duration);
    cameraPreviewAnimationDuration =
        getResources().getInteger(R.integer.camera__preview__ainmation__duration);
  ...
  private void disableCameraButtons() {
    ...
    ViewUtils.getView(getView(), R.id.gtv__camera__top_control__change_camera)
        .setVisibility(View.GONE);
    ViewUtils.getView(getView(), R.id.gtv__camera__top_control__flash_setting)
        .setVisibility(View.GONE);
  }
  ...
  @Override
  public void onSketchOnPreviewPicture(
      ImageAsset imageAsset,
      ImagePreviewLayout.Source source,
      IDrawingController.DrawingMethod method) {
    getControllerFactory()
        .getDrawingController()
        .showDrawing(imageAsset, IDrawingController.DrawingDestination.CAMERA_PREVIEW_VIEW);
  }
  ...
  public void onSendPictureFromPreview(ImageAsset imageAsset, ImagePreviewLayout.Source source) {
    getControllerFactory()
        .getCameraController()
        .onBitmapSelected(imageAsset, pictureFromCamera, cameraContext);
  }
  ...
  private void showPreview(ImageAsset imageAsset, boolean bitmapFromCamera) {
    ...
    final ImagePreviewLayout imagePreviewLayout =
        (ImagePreviewLayout)
            LayoutInflater.from(getContext())
                .inflate(R.layout.fragment_cursor_images_preview, imagePreviewContainer, false);
    ...
    imagePreviewLayout.setAccentColor(
    ...
    ObjectAnimator.ofFloat(imagePreviewContainer, View.ALPHA, 0, 1)
        .setDuration(cameraPreviewAnimationDuration)
        .start();
    cameraBottomControl.setVisibility(View.GONE);
  }

  private void hideImagePreviewOnAnimationEnd() {
    if (imagePreviewContainer != null && cameraBottomControl != null) {
      imagePreviewContainer.setVisibility(View.GONE);
