...
public class GuidedEditText extends LinearLayout {
  ...
  public void showMessage(boolean show) {
    ...
    if (show) {
      ...
      ValueAnimator.ofObject(new HeightEvaluator(guidanceText), 0, guidanceHeight)
          .setDuration(duration)
          .start();
    } else {
      guidanceText.setVisibility(GONE);
      ValueAnimator.ofObject(new HeightEvaluator(guidanceText), guidanceHeight, 0)
          .setDuration(getResources().getInteger(R.integer.profile__guidance__animation__duration))
          .start();
    }
  }
