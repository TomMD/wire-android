...
public class PhoneConfirmationButton extends GlyphTextView {
  ...
  public PhoneConfirmationButton(Context context, AttributeSet attrs, int defStyle) {
    ...
    cornerRadius =
        getResources().getDimensionPixelSize(R.dimen.new_reg__phone_number__corner_radius);
    errorCircleRadius =
        getResources()
            .getDimensionPixelSize(R.dimen.new_reg__sign_up__invalid_phone__circle_radius);
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (state == State.INVALID) {
      canvas.drawCircle(
          canvas.getWidth() / 2, canvas.getHeight() / 2, errorCircleRadius, circlePaint);
    }
  }
