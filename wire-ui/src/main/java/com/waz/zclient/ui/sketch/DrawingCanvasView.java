...
public class DrawingCanvasView extends View {
  ...
  private final int defaultStrokeWidth =
      getResources().getDimensionPixelSize(R.dimen.color_picker_small_dot_radius) * 2;
      ...
      new GestureDetector(
          ...
          new GestureDetector.SimpleOnGestureListener() {
            public void onLongPress(MotionEvent e) {
              ...
              canvasHistory.addFillScreen(
                  bitmap.getWidth(), bitmap.getHeight(), new Paint(drawingPaint));
              paintedOn(true);
              drawingPaint.setStyle(Paint.Style.STROKE);
              invalidate();
            }
  ...
  public Rect getImageTrimValues() {
    ...
    return new Rect(
        Math.max(0, left - leftTrimBuffer),
  ...
  public boolean undo() {
    ...
    if (last instanceof SketchCanvasHistory.Text) {
      ...
      if (newLastText != null && newLastText.text != null) {
        drawingCanvasCallback.onTextChanged(
            newLastText.text, (int) newLastText.x, (int) newLastText.y, newLastText.scale);
      } else {
