...
public class CursorIconButton extends GlyphTextView {
  ...
  private void setBackgroundColor(int defaultColor, int pressedColor) {
    ...
    float avg =
        (Color.red(pressedColor) + Color.blue(pressedColor) + Color.green(pressedColor))
            / (3 * 255.0f);
    if (avg > TRESHOLD) {
      float darken = 1.0f - DARKEN_FACTOR;
      pressedColor =
          Color.rgb(
              (int) (Color.red(pressedColor) * darken),
    ...
    GradientDrawable pressedBgColor =
        new GradientDrawable(GradientDrawable.Orientation.BOTTOM_TOP, new int[] {pressed, pressed});
    pressedBgColor.setShape(GradientDrawable.OVAL);

    GradientDrawable defaultBgColor =
        new GradientDrawable(
            GradientDrawable.Orientation.BOTTOM_TOP, new int[] {defaultColor, defaultColor});
  ...
  public void initTextColor(int selectedColor) {
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
      ...
      pressedColor =
          getResources().getColor(R.color.text__primary_dark_40, getContext().getTheme());
      focusedColor = pressedColor;
      enabledColor = getResources().getColor(R.color.text__primary_dark, getContext().getTheme());
      disabledColor =
          getResources().getColor(R.color.text__primary_dark_16, getContext().getTheme());

      if (!ThemeUtils.isDarkTheme(getContext())) {
        pressedColor =
            getResources().getColor(R.color.text__primary_light__40, getContext().getTheme());
        focusedColor = pressedColor;
        enabledColor =
            getResources().getColor(R.color.text__primary_light, getContext().getTheme());
        disabledColor =
            getResources().getColor(R.color.text__primary_light_16, getContext().getTheme());
      }
    }
