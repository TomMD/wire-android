...
public class SpannableEditText extends AccentColorEditText {
  ...
  public abstract static class TokenSpan extends ReplacementSpan {
          ...
          public boolean onTouch(View v, MotionEvent event) {
            ...
            if (event.getAction() == MotionEvent.ACTION_UP) {
              float eventDistance =
                  (float)
                      Math.sqrt(
                          (gestureStartX - event.getX()) * (gestureStartX - event.getX())
                              + (gestureStartY - event.getY()) * (gestureStartY - event.getY()));
              float distanceTolerance =
                  ViewUtils.toPx(getContext(), TOKEN_CLICK_EVENT_DISTANCE_TOLERANCE_DP);
