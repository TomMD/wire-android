...
/**
 * The PullToRefreshContainer is a FrameLayout that eats up all touch events and in case of an
 * overscroll called by the BouncingListView it translates the ListView to the overscroll position.
 * If ACTION_UP happens or the Overscroll offset goes into another direction the TouchEvent is
 * dispatched to the BouncingListView.
 */
public class PullForActionContainer extends FrameLayout
    implements OverScrollListener, OffsetAnimateable {
  /** class tag */
  ...
  public void setPullForActionView(
      @NonNull PullForActionView pullForActionView, FillType fillType) {
    ...
    addView(
        (View) this.pullForActionView,
        new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, heightType));
  }
  ...
  /**
   * This method eats all TouchEvents and during a gesture it will not be called again. If the first
   * event is already an overscroll event the LastY should already be available.
   *
   * @return always true
   */
  @Override
  ...
  /**
   * If the ListView is not overscrolling the TouchEvent will be dispatched to the ListView.
   * Otherwise an bouncing logic is implemented.
   *
   * @return always true
   */
  @Override
    ...
    if (isDown) {
      switch (mode) {
        case NONE:
          ...
          break;
        case BOTTOM:
          if (pullForActionMode.equals(PullForActionMode.BOTTOM)
              || pullForActionMode.equals(PullForActionMode.TOP_AND_BOTTOM)) {
            overScrollMode = mode;
          } else {
            overScrollMode = OverScrollMode.NONE;
          }
          break;
        case TOP:
          if (pullForActionMode.equals(PullForActionMode.TOP)
              || pullForActionMode.equals(PullForActionMode.TOP_AND_BOTTOM)) {
            overScrollMode = mode;
          } else {
            overScrollMode = OverScrollMode.NONE;
