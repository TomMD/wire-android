...
public class ColorPickerLayout extends LinearLayout {
  ...
  private void populateDotsView(int selectedColor) {
    ...
    for (int accentColor : accentColors) {
      final ColorPickerDotView dot =
          (ColorPickerDotView) layoutInflater.inflate(R.layout.color_picker_dot_view, this, false);
      ...
      if (colorPickerView != view) {
        ...
        if (view instanceof ColorPickerDotView) {
          ...
          onColorSelectedListener.onColorSelected(
              colorPickerDotView.getCircleColor(), colorPickerDotView.getStrokeSize());
        }
      }
    }
    invalidate();
