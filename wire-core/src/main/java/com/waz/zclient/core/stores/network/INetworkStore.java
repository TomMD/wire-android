...
public interface INetworkStore extends IStore {
  ...
  /**
   * A null NetworkAction implies that no work will be done if there is a connection, but the user
   * will be notified if there is no connection.
   *
   * @param networkAction
   *     <p>TODO this null hack (or the old `notifyNoInternet` method are only really needed because
   *     SE doesn't tell us when something failed TODO due to no connection - perhaps we can have
   *     that passed down through the ConnectionIndicator?
   */
