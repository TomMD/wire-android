...
public class ScalaNetworkStore implements INetworkStore {
  ...
  private final ModelObserver<ConnectionIndicator> connectionIndicatorModelObserver =
      new ModelObserver<ConnectionIndicator>() {
        @Override
        public void updated(ConnectionIndicator model) {
          Timber.i(
              "NETWORK_TRACE: ConnectionIndicator updated, net=%s, error=%b, websocket=%b",
              model.getNetworkMode().toString(),
              model.isConnectionError(),
              model.isWebSocketConnected());
          networkMode = model.getNetworkMode();
          isServerError =
              hasInternetConnection()
                  && model
                      .isConnectionError(); // only care about server errors when there is an
                                            // internet connection
  ...
  /**
   * We want to notify the user if there is a server connection error OR no internet connection, but
   * we want to try and perform the action if there is internet but no server connection.
   */
  @Override
